    ...
    Column(
        ...
        Row(
            ...
            OutlinedTextField(
                ...
                onValueChange = {
                    val isTextChange = message.text != it.text
                    val msg = if (state.sendFormat == 0) {
                        val text = filterHex(message.text, it.text)
                        it.copy(
                            text = text,
                            selection = if (isTextChange) TextRange(text.length) else it.selection
                        )
                    } else it
                    onAction(BottomBarAction.OnMessageChange(msg))
                },
